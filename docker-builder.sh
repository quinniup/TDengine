...
cmd="docker buildx build --platform=linux/arm/v7 -t ${IMAGE}-builder-${ARCH}:${B_VER} --load -f Dockerfile.builder ."
echo $cmd

cmd="docker buildx build --platform=linux/arm/v7 --build-arg BASE=${IMAGE}-builder-${ARCH}:${B_VER} -t ${IMAGE}-compile-${ARCH}:${C_VER} --load -f Dockerfile.compile ."
echo $cmd

cmd="docker buildx build --platform=linux/arm/v7 --build-arg BASE=${IMAGE}-compile-${ARCH}:${C_VER} -t ${IMAGE}-${ARCH}:${VER} --load -f Dockerfile ."
echo $cmd
